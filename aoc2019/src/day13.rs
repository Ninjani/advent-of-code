...
impl<'a> AocDay<'a> for Day13 {
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        let mut arcade = ArcadeCabinet::new(input.to_vec());
        arcade.run()?;
        Ok(arcade
            .tiles
            .iter()
...
impl ArcadeCabinet {
    ...
    fn step(&mut self) -> color_eyre::Result<()> {
        loop {
...
fn write_to_file(filename: &str, contents: &str) -> color_eyre::Result<()> {
    let mut f = File::create(filename)?;
    f.write_all(contents.as_bytes())?;
    Ok(())
}

fn play(arcade: &mut ArcadeCabinet, read: bool, write: bool) -> color_eyre::Result<i64> {
    if read {
        let joysticks = fs::read_to_string("data/2019/day_13.output")?
            .trim()
            .split(',')
            .map(|i| i.parse::<i64>())
            .collect::<Result<Vec<_>, _>>()?;
