...
impl<'a> AocDay<'a> for Day11 {
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        let mut robot = PaintingRobot::new(IntCode::new(input.to_vec(), 0), Color::Black)?;
        robot.run()?;
        Ok(robot.grid.len())
    }

    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        let mut robot = PaintingRobot::new(IntCode::new(input.to_vec(), 0), Color::White)?;
        robot.run()?;
...
impl PaintingRobot {
    fn new(program: IntCode, start_color: Color) -> color_eyre::Result<Self> {
    ...
    fn paint(&mut self) -> color_eyre::Result<()> {
