...
impl Parameter {
    fn _get_address(&self, program: &[i64]) -> color_eyre::Result<i64> {
        if self.value < 0 {
            Err(eyre!("negative address"))
        } else {
    ...
    fn _set_address(&self, value: i64, program: &mut Vec<i64>) -> color_eyre::Result<()> {
        if self.value < 0 {
            Err(eyre!("negative address"))
        } else {
            let address = self.value as usize;
    ...
    fn get(&mut self, program: &[i64], relative_base: i64) -> color_eyre::Result<i64> {
        match self.mode {
    ...
    fn set(
        ...
        match self.mode {
            ...
            _ => Err(eyre!("set parameter must be in position/relative mode")),
        }
    }
}
...
impl IntCode {
    ...
    pub fn process(&mut self) -> color_eyre::Result<Option<i64>> {
        while !self.step()? {}
        Ok(self.get_last_output())
    }

    fn step(&mut self) -> color_eyre::Result<bool> {
        let (opcode, mut parameters) = self.make_instruction()?;
        Ok(self.process_instruction(opcode, &mut parameters)?)
    }

    pub fn make_instruction(&mut self) -> color_eyre::Result<(OpCode, Vec<Parameter>)> {
        ...
        let opcode: OpCode = FromPrimitive::from_usize(opcode_int)
            .ok_or_else(|| eyre!("Unknown OpCode {}", opcode_int))?;
            ...
            .rev()
        {
            parameters.push(Parameter {
                ...
                mode: FromPrimitive::from_u32(
                    char.to_digit(10).ok_or_else(|| eyre!("char not a digit"))?,
                )
                .ok_or_else(|| eyre!("Unknown Mode"))?,
            });
        }
