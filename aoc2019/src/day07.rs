...
impl<'a> AocDay<'a> for Day07 {
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        ...
        for phase_sequence in (0..5).permutations(5) {
            let p_output = get_thrust(input, &phase_sequence)?;
            if p_output > output {
                output = p_output
            }
        }
    ...
    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        ...
        for phase_sequence in (5..10).permutations(5) {
            let p_output = get_thrust_feedback(input, &phase_sequence)?;
            if p_output > output {
                output = p_output
            }
        }
...
impl IntCode {
    fn process_with_two_inputs(&mut self, new_input: i64) -> color_eyre::Result<Option<i64>> {
        loop {
            let (opcode, mut parameters) = self.make_instruction()?;
            if self.process_instruction(opcode, &mut parameters)? {
                break;
            }
    ...
    fn process_till_next_output(&mut self, new_input: i64) -> color_eyre::Result<Option<i64>> {
        loop {
            let (opcode, mut parameters) = self.make_instruction()?;
            if self.first_input_done && opcode == OpCode::Input {
                self.input_value = new_input;
            }
            self.process_instruction(opcode, &mut parameters)?;
            if !self.first_input_done && opcode == OpCode::Input {
                self.input_value = new_input;
                self.first_input_done = true;
            }
...
fn get_thrust(program: &[i64], phase_sequence: &[i64]) -> color_eyre::Result<i64> {
...
pub fn get_thrust_feedback(program: &[i64], phase_sequence: &[i64]) -> color_eyre::Result<i64> {
    let mut new_input = 0;
...
mod tests {
    ...
    fn test_1() -> color_eyre::Result<()> {
        let program = Day07::load("3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0")?;
        assert_eq!(43210, get_thrust(&program, &[4, 3, 2, 1, 0])?);
        let program = Day07::load(
            "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0",
        )?;
        assert_eq!(54321, get_thrust(&program, &[0, 1, 2, 3, 4])?);
        let program = Day07::load("3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0")?;
    ...
    fn test_2() -> color_eyre::Result<()> {
        ...
        let program = Day07::load("3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10")?;
