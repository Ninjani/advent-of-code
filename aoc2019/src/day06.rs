...
impl<'a> AocDay<'a> for Day06 {
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        Ok(input
            .get_distances_to_start(input.node_to_index["COM"])
            .into_iter()
            .filter_map(|i| i)
            .sum())
    }
    ...
    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        Ok(input
            .get_distance_n1_to_n2(input.node_to_index["YOU"], input.node_to_index["SAN"])
            .unwrap()
            - 2)
    }
}

pub struct Tree<T: Hash + Clone + Eq + Debug> {
