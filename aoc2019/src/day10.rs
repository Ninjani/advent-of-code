...
impl<'a> AocDay<'a> for Day10 {
    ...
    fn load(input: &str) -> color_eyre::Result<Self::Input> {
        Ok(parse_asteroid_map(input)?)
    }

    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        Ok(get_asteroid_with_most_visible(input)?.1)
    }

    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        let center = get_asteroid_with_most_visible(input)?.0;
        let center_index = (0..input.len()).find(|i| input[*i] == center).unwrap();
        let asteroid_200 = shoot_asteroids_in_order(center_index, input)[&200];
...
fn parse_asteroid_map(input: &str) -> color_eyre::Result<Vec<Asteroid>> {
    Ok(input
        .split('\n')
        .enumerate()
        .flat_map(|(j, line)| {
...
fn get_asteroid_with_most_visible(asteroids: &[Asteroid]) -> color_eyre::Result<(Asteroid, usize)> {
