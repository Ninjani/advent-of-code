...
impl<'a> AocDay<'a> for Day03 {
    ...
    fn load(input: &str) -> color_eyre::Result<Self::Input> {
        ...
        let wire_2 = get_wire_points(
            &lines
                .next()
                .ok_or_else(|| eyre!("Input needs two lines"))?
                .split(',')
                .collect::<Vec<_>>(),
        )?;
        Ok((wire_1, wire_2))
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        Ok(get_closest_intersection_manhattan(&input.0, &input.1))
    }

    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        Ok(get_closest_intersection_steps(&input.0, &input.1))
    }
}
...
fn parse_movement(movement: &str) -> color_eyre::Result<Vec<Point>> {
...
fn get_wire_points(directions: &[&str]) -> color_eyre::Result<Wire> {
...
mod tests {
    ...
    fn test_1() -> color_eyre::Result<()> {
        assert_eq!(6, Day03::part_1(&Day03::load("R8,U5,L5,D3\nU7,R6,D4,L4")?)?);
        assert_eq!(
            159,
            Day03::part_1(&Day03::load(
    ...
    fn test_2() -> color_eyre::Result<()> {
        assert_eq!(
            30,
            Day03::part_2(&Day03::load("R8,U5,L5,D3\nU7,R6,D4,L4")?)?
        );
        assert_eq!(
            610,
            Day03::part_2(&Day03::load(
                "R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83"
            )?)?
        );
        assert_eq!(
