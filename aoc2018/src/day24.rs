    ...
    fn part_2(input: &Self::Input) -> color_eyre::Result<Self::Result2> {
        for boost in 30.. {
            ...
            if battle.infection.is_empty() {
                return Ok(battle.immune_system.values().map(|g| g.units).sum());
            }
        }
        Ok(0)
    }
...
impl Group {
    ...
    pub fn choose_target(&self, enemies: &HashMap<usize, &Group>) -> Option<usize> {
        let enemies: Vec<_> = enemies
            .iter()
            .filter(|(_, enemy)| self.calculate_damage(enemy) > 0)
            .collect();
        if enemies.is_empty() {
            None
        } else {
...
impl Battle {
    pub fn new(immune_system_groups: Vec<Group>, infection_groups: Vec<Group>) -> Self {
        ...
        Battle {
            immune_system: immune_system_groups.into_iter().enumerate().collect(),
            infection: infection_groups
                .into_iter()
                .enumerate()
                .map(|(i, g)| (i + num_immune_system_groups, g))
                .collect(),
        }
    }
    ...
    pub fn target_selection(&self) -> HashMap<usize, usize> {
        let order = self
            ...
            .sorted_by(|b, a| {
                (a.1.effective_power(), a.1.initiative)
                    .cmp(&(b.1.effective_power(), b.1.initiative))
            });
    ...
    pub fn attack(&mut self, targets: &HashMap<usize, usize>) {
        let order = targets.keys().sorted_by(|b, a| {
            let a_group = if self.immune_system.contains_key(a) {
                self.immune_system[a].initiative
            } else {
                self.infection[a].initiative
            };
            let b_group = if self.immune_system.contains_key(b) {
                self.immune_system[b].initiative
            } else {
                self.infection[b].initiative
            };
            a_group.cmp(&b_group)
        });
        for unit in order {
            ...
            if !self.immune_system.contains_key(&enemy_index)
                && !self.infection.contains_key(&enemy_index)
            {
                continue;
            }
            let against_infection = if self.immune_system.contains_key(unit) {
                true
            } else {
                false
            };
