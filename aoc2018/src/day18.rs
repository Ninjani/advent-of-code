...
impl<'a> AocDay<'a> for Day18 {
    type Input = LumberCollectionArea;
    type Result1 = usize;
    type Result2 = usize;
    ...
    fn load(input: &str) -> color_eyre::Result<Self::Input> {
        Ok(LumberCollectionArea::read(input))
    }
    ...
    fn part_1(input: &Self::Input) -> color_eyre::Result<Self::Result1> {
        ...
        let (num_trees, num_lumberyards) = input.get_counts();
        Ok(num_trees * num_lumberyards)
    }
