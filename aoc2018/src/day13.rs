...
impl CartSystem {
    fn new(input: &str) -> color_eyre::Result<CartSystem> {
        ...
        for (i, row) in input.split('\n').enumerate() {
            ...
            for (j, c) in new_row.chars().enumerate() {
                tracks.insert((i, j), c.to_string().parse()?);
                if "<>^v".contains(c) {
                    carts.push((i, j, c.to_string().parse()?, 1));
                }
            }
        }
